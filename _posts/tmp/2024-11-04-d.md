...
이때 normalizing constant를 계산하기가 어렵다.
...
Maximum likelihood training의 목적은 모델 분포와 데이터의 분포의 KLD를 $\theta$에 대해 최소화하는 것이다. 하지만 그것이 어렵기 때문에 모델 분포의 스코어와 데이터 분포의 스코어에 대한 차이를 최소화하자. score를 사용한다면 normalizing constant를 무시할 수 있기 때문이다.
...
Problem: Jacobian trace of the score network is expensive to compute.
