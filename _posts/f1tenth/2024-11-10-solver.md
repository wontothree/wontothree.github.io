...
- $\hat{\mathbf{U}} = [\hat{\mathbf{u}}]_{\tau = 0}^{T-1}$: the initial estimated control input sequence, often using the previous optimal solution
- $\mathbf{V}_k = [\mathbf{v}_{\tau}]_{\tau = 0}^{T-1} \in \mathbb{R}^{T \times m}$: randomly generated control input sequences
