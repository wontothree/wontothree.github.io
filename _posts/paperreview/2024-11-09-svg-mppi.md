...
The standard MPPI algorithm minimizes Forward KL (FKL) divergence, denoted as $\mathbb{D}_{\text{KL}} (\mathbb{Q}^* \vert\vert \mathbb{Q})$, where $\mathbb{Q}$ may cover the entire $\mathbb{Q}^*$.

In constrast, when the input arguments are reversed, the Reverse KL (RKL) divergence $\mathbb{D}_{\text{KL}} (\mathbb{Q} \vert\vert \mathbb{Q}^*)$ exhibits a difference characteristic, where $\mathbb{Q}$ converges to a single mode of the optimal distribution $\mathbb{Q}^*$
...
Our proposed method first spreads $\mathbf{K}_g$ guide particles $[\mathbf{V}_k^g]_{k=0}^{K_g-1} \in \mathbb{R}^{\mathbf{K}_g \times T \times m}$ in addition to the $\mathbf{K}$ samples used in the standard MPPI algorithm.
