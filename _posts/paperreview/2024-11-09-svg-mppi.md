...
The optimal control input sequence $\mathbf{U}_t^*$ is obtained by sampling $\mathbf{V}_k$ from the optimal action distribution $\mathbb{Q}^*$ and taking its expected value. However since sampling directly from $\mathbb{Q}^*$ is not possible, important sampling is used with the 
