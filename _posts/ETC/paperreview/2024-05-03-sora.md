...
- 3.4 Language Instruction Following은 사용자의 prompt 입력에 적절한 이미지를 생성하기 위해 text-image pair로 모델을 학습시키는 과정에 대해 다룬다. 이때 text-image pair를 생성하는 과정에서 사람이 아니라 어떤 model을 이용한다.
