...
$$
<\mathbb{f}, \mathbb{g}>_{\mathcal{H}} = \sum_{ij} a_ib_j k(x_i, x_j)
$$

for $g(x) = \sum_i b_i k(x, x_i)$.

$\mathcal{H}^d$ : space of vector functions $\mathbb{f} = [f_1, \dots, f_d]$ with $f_i \in \mathcal{H}$, equipped with inner product

$$<\mathbb{f}, \mathbb{g}>_{\mathcal{H}^d} = \sum_{i=1}^d <f_i, g_i>_{\mathcal{H}}$$.
