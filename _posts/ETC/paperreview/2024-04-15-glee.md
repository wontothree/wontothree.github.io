...
|Object decoder|we build the object decoder upon MaskDINO [50] with a dynamic class head by compute similarity between object embedding from detector and text embedding from the text encoder.|
...
||Then we feed them into the object decoder and adopt <span style='color: #2D3748; background-color: #ffdce0'>three prediction heads (classification, detection, and segmentation)<\span> on the <span style='color: #2D3748; background-color: #ffdce0'>output embedding $q_d = \mathbb{R}^{N \times C}$ from decoder<\span>.|
