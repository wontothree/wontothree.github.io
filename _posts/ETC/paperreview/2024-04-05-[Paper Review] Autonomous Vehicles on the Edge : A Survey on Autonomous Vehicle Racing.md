...
- Global planner에서는 non-linear optimal control problem을 푸는 방식을 이용할 수 있고 local planner에서는 Model predictive control을 이용할 수 있는데, 둘은 어떤 차이가 있으며 각각을 이용하는 이유가 뭘까?
  여기서 mpc는 실시간성이 강조되고 optimal control은 좀 더 긴 시간에 대해 뜨문뜨문 푼다. 한 바퀴 돌 때마다 global planner를 돌릴 수도 있다. local planner의 입장에서는 대략적인 선이 있다. global planner의 역할을 단순화하면 중앙선을 따는 것으로 생각할 수 있다. global planner에 대해 강화학습을 적용한 연구가 있었다.

  최적제어 문제도 최적화 문제의 일종이다. 풀고 싶은 최적화 문제는 동일하다. 푸는 솔버에 따라 구하는 방식이 달라진다. 어떻게 하면 더 빠르게 최적의 값을 찾을 수 있을까. mpc의 틀은 놔두고 그걸 어떻게 풀지에 집중한다.

  원래 최적화 문제는 시간에 대한 이야기가 없다. 최적제어는 기본적으로 시간에 대한 고려가 있다.

  찾고자 하는 것은 입력이다. mpc는 매 시간마다 새로 푼다. 틀렸을 수 있으니까 나머지는 폐기한다. 매 순간 풀어서 입력을 매순간 찾는다.


mpc를 구현하는 과정에서는 최적화 솔버를 쓴다.

numerical한 solver를 슨다.

처음부터 이산 시각이라고 가정하고 시작한다. laplace는 비선형 시스템에서 사용하지 않는다.

MPCC 0~10 다 풀고 1~10 은 버린다. 과연 이것이 옳은 일인가? 좀 써먹어도 되지 않을까? k=1에서 동일하게 풀지만 버린 애들도 꾀 최적일 것이라 생각하여 다음 애들의 초기값으로 사용한다.


latency

0,2라고 하면 그 사이에 뭘 해야 한다.

mpc를 마지막 보루라고 생각하고 zero order hold로 한다. 사이를 매꿔줄

잡다한 걸 고려하기 위한 컨트롤러가 존재할 수 있다.

mpc는 가능성을 뿌려놓고 생각하지 않는다. (해비)

mppi : sampling 전략과 합친 것

sampling-based~

미리 뿌려놓고 선택을 하는 문제로 바꾼다. sampling을 잘 뿌리는 게 이슈다. 솔버는 알려진 걸 쓸 것이다. 옵션을 조정하는 거다. gd를 쓸 수도 있다.



- Modeling이라는 것이 system identification을 말하는 것일까?
  선형 시스템에서 A, B, C, D를 찾는 게 system identification : input, output만 있을 때 동역학 정보가 없을 때..

기계과에서는 동역학으로부터 A, b, c,d를 찾는다.

방식이 2가지

urdf는 기구학적인 걸 확용하자. 모델링에 대한 정보를 제공한다.

모터에 대해 identification 이 필요할 수 있다. 동역학 모델대로 돌아가지 않을때
