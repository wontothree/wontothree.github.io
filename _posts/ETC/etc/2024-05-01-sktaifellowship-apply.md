...
1-1. 이미지에서 Yolo 9000 classes에 해당하는 객체 추출한다.
1-2. 이미지의 모든 객체와 객체의 속성, 배경에 대한 정보를 text로 저장한다.
2-1. Image frame을 scene 단위로 묶는다.
2-2. 각 Scene에서 동작을 text로 저장한다.
3-1. 사용자가 질문을 한다.
3-2. 질문의 문장에서 '탐색의 단서 정보'와 '요구 사항'을 각각 추출한다.
3-3. '탐색의 단서 정보'에서 배경, 객체, 객체의 속성, 동작을 각각 추출한다.
4-1. '탐색의 단서 정보'에서 추출된 배경, 객체, 객체의 속성, 동작에 대한 데이터를 바탕으로 관련이 있는 scene을 탐색한다.
