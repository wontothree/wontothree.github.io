...
- 영상 데이터를 분석하고 검색 시스템을 만든다.
- 영상 검색 시스템에 AI를 활용할 수 있는 획기적인 방안을 찾아내고, 실제 서비스에 활용할 수 있는 가능성을 발굴한다.
- 실시간 관제 시스템의 검색 기능
- 실시간 영상을 AI 모델로 분석, 다양한 객체의 속성이나 이벤트를 사용자에게 제공하는 분석 모듈 개발 중
- 실시간 비디오 분석을 통해 사람, 차량, 성별, 연령, 차종, 행동 인식, 화재, 추락, 침입, 배회, 쓰레기 투기 등 분석
- [기존의 문제] 영상 분석을 통해 정해지지 않은 유의미한 정보 도출 -> Scene에 대한 분석이나 Video 자체에 대한 분석 등 좋은 raw data를 뽑는 모델이 함께 연구되어야 한다.
- 목표
  1. 영상으로부터 유용하고 다양한 데이터 저장 및 추출
  2. LLM이 잘 이해할 수 있도록 취득한 데이터 가공
  3. 질문과 가공된 데이터를 입력하여 영상으로부터 원하는 정보를 취득
- 프로젝트
  1. 비디오 검색에 필요한 작업과 데이터가 무엇일지 분석 및 연구를 진행한다.
  2. 데이터를 추출하고 이를 DB에 적재한다.
  3. DB로부터 데이터 추출 및 LLM에 활용할 수 있게 가공한다.
  4. 질문, 가공된 데이터를 LLM에 입력하고 원하는 정보를 제공받는다.
